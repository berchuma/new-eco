 ...
 /* this makes the code to loop bc it render with every chang in curentvalue*/
  useEffect(() => {
    let slider = setInterval(() => {
        console.log()
      if (currentIndex < 2) {/*how much content it contens and it conens 3 value*/
        setCurrentIndex(currentIndex + 1);/* the increment*/
        setTranslateValue(-(currentIndex + 1) * 125);/* the value of transletion*/
      }
      /* if it is greter*/
      else {
        setCurrentIndex(0);
  ...
  return (
    ...
    <div className={style.slider}>{/*the main cotener */}
